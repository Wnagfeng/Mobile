...
<template>
    <div class="warpper">
        ...
        <!-- 优惠券选择 -->
        <!-- <van-cell title="优惠券" is-link @click="showList = true">
        ...
        <van-popup v-model="showList" round position="bottom" :style="{ height: '30%', background: '#f5f5f5' }">
            <van-cell center class="discount" v-for="(item, index) in couponList" :key="index" @click="ckCoupon(item)">
                <template #title>
                ...
                <template #label>
                    <font size="3" color="#ff5a5f">{{ item.endTime }} 到期</font>
                </template>
                <template #icon>
        ...
        <!-- 价格信息 -->
        <van-cell title="商品价格" :value="priceInfo.result.totalPrice" />
        <van-cell title="优惠" :value="priceInfo.result.couponPrice" />
        ...
        <van-submit-bar :price="priceInfo.result.payPrice * 100" button-text="提交订单" @submit="createOrder" />
    </div>
</template>
